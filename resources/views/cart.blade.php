            ...
            @if(session('cart'))
                <div class="bg-gray-300 rounded-lg shadow-md p-6">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <form action="{{ route('cart.remove') }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="id" value="{{ $id }}">
                                            <button type="submit" class="text-primary hover:text-primary-700">Remove</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <div class="mt-6 text-right">
                        ...
                        <a href="/checkout" class="mt-4 inline-block bg-primary hover:bg-primary-700 text-white px-5 py-3 rounded">Proceed to Checkout</a>
                    </div>
                </div>
            @else
